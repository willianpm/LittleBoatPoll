...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    try {
      ...
      const pesoInfo = usarPesoMensalista ? '👑 Mensalistas têm peso 2 nos votos' : '⚖️ Todos têm o mesmo peso';
      const pollEmbed = new EmbedBuilder()
        .setColor('#FFD700') // Ouro
        .setTitle(`📚 ${titulo}`)
        .setDescription(descricao)
        .addFields({ name: '📊 Regras', value: `• Você pode votar em até **${maxVotos}** opç${maxVotos > 1 ? 'ões' : 'ão'}\n• ${pesoInfo}`, inline: false })
      ...
      const updatedEmbed = EmbedBuilder.from(pollEmbed).addFields({ name: 'ID', value: `\`${msg.id}\``, inline: true });
